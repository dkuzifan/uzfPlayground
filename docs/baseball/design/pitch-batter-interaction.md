...
> **선구안(Eye)**: ✅ **구현 완료 (2026-04-07)**. `PlayerStats.eye?: number` (기본값 60). v2 타격 모델에서 투구 인식 정확도(구종/코스)에 사용. `read-pitch.ts` → `readPitch(pitch, prediction, eye)`.
...
> **⚠️ v2 구현 (2026-04-07)**: 땅볼은 최종 정지점이 아닌 **경로 기반 인터셉트** 모델 적용.
> 수비수→경로 수직거리 계산 → 공이 해당 지점 도달 시간 vs 수비수 도달 시간 비교.
> 4구간 포구 모델: 여유(margin>0.3s)/빠듯(0~0.3s)/반응캐치(-0.2~0s)/불가(<-0.2s).
> 뜬공도 동일한 시간 기반 마진으로 통일. 상세: `catch-probability.ts` → `calcCatch4Zone()`.
...
> **⚠️ v2 변경 (2026-04-07)**: 아래 `P(fair | contact)`는 **v1 레거시**. v2에서는 방향각(θ)으로 판정:
> - `|θ| ≤ 45°` → 페어
> - `45° < |θ| ≤ 60°` → 수비 가능 파울 (뜬공이면 파울 플라이 아웃 가능)
> - `|θ| > 60°` → 관중석 파울 (순수 파울)
> - 파울 방향 분포: 지수 감소 (λ=0.012), 파울 라인 근처 집중
>
> `fair_prob` 값은 v1 fallback 및 `selectDirectionAngle` 페어/파울 분기 확률로만 사용.
...
> **파울팁 처리 (v2)**: 2스트라이크 + 순수 파울 시 5% 확률로 파울팁 → 스트라이크(삼진) 처리. 파울 플라이 아웃 시 태그업 가능. 파울 플라이 에러 시 파울 처리 + 오버레이.
