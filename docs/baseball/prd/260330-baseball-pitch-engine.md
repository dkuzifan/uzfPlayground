...
```
chase/dirt 비중 대폭 증가 → 유인구 투척
  slider → B22,B24,B26 가중치 ×2.0
  splitter/curve → B33 가중치 ×2.0
  core 거의 사용 안 함 → "낭비구 or 유인구"
```

### 투수 불리 (3B)

```
edge 집중 → 볼넷 방지하면서 치기 어려운 코스
  core 허용 (카운트 세기 위해)
  ball/dirt 억제
```
...
Step 3: zone-select.ts 카운트별 전략 강화
Step 4: 캘리브레이션 (K%, 투구/PA, 존 분포)
Step 5: (추후) 구종별 착각(deception) 모델 기획 및 구현
```
